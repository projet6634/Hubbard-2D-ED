...
c     
     ...
     &           '_neupd: Last row of the Schur vector matrix')
            if (msglvl .gt. 3) then
               call pdmout (comm, logfil, ncv, ncv, workl(iuptri), ldh, ndigit,
...
c
     ...
     &           '_neupd: Imag part of the eigenvalues of H--reordered')
                if (msglvl .gt. 3) then
                   call pdmout (comm, logfil, ncv, ncv, workl(iuptri), ldq, 
     &                  ndigit,
...
c
     ...
     &              '_neupd: Last row of the eigenvector matrix for T')
               if (msglvl .gt. 3) then
                  call pdmout (comm, logfil, ncv, ncv, workl(invsub), ldq, 
