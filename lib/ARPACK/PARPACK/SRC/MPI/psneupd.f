...
c\Usage:
c  call psneupd 
c     ( COMM, RVEC, HOWMNY, SELECT, DR, DI, Z, LDZ, SIGMAR, SIGMAI, WORKEV, BMAT, 
...
c
c\SCCS Information:
c FILE: neupd.F   SID: 1.4   DATE OF SID: 8/13/96
c
c\EndLib
c
c-----------------------------------------------------------------------
      subroutine psneupd 
     &         (comm, rvec, howmny, select, dr, di, z, ldz, sigmar, 
...
c     %----------------------%
c
      external   scopy, sger, sgeqr2, slacpy, slahqr, slaset, psmout, 
     &           sorm2r, strevc, strmm, strsen, sscal, psvout, pivout
c
c     %--------------------%
...
c
         if (msglvl .gt. 2) then
            call psvout(comm, logfil, 1, thres, ndigit,
...
c     
     ...
     &           '_neupd: Last row of the Schur vector matrix')
            if (msglvl .gt. 3) then
               call psmout (comm, logfil, ncv, ncv, workl(iuptri), ldh, ndigit,
...
c
     ...
     &           '_neupd: Imag part of the eigenvalues of H--reordered')
                if (msglvl .gt. 3) then
                   call psmout (comm, logfil, ncv, ncv, workl(iuptri), ldq, 
     &                  ndigit,
...
c
     ...
     &              '_neupd: Last row of the eigenvector matrix for T')
               if (msglvl .gt. 3) then
                  call psmout (comm, logfil, ncv, ncv, workl(invsub), ldq, 
