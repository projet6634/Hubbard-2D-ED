...
c
c\SCCS Information:
c FILE: seupd.F   SID: 1.6   DATE OF SID: 3/19/97
c
c\EndLib
c
c-----------------------------------------------------------------------
...
c
             if (msglvl .gt. 2) then
                call pdvout(comm, logfil, 1, thres1, ndigit,
...
c
             if (msglvl .gt. 2) then
                kv(1) = thres1
                kv(2) = thres2
                call pdvout(comm, logfil, 2, kv, ndigit,
     &          '_neupd: Threshold eigenvalues used for re-ordering')
             end if
c
         end if
c
